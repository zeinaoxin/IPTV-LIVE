...
def _read_repo_secret(name: str) -> Optional[str]:
    """
    从环境变量读取仓库 Secret（优先）
    - GITHUB_TOKEN 是 Actions 内置
    - 自定义 Secret 会被注入为环境变量
    """
...
def _verify_token_once(token: str) -> bool:
    """请求一个 taoiptv lives 接口，确认 Token 不是“Authentication Failed”"""
    try:
...
def get_taoiptv_token() -> Optional[str]:
    """
    统一入口
    - 优先从仓库 Secrets 读取（REPO_SECRET_TAOTOKEN）
    - CI 环境下若未配置密钥，直接跳过自动化（避免长时间卡在挑战页）
    - 非 CI 环境才会尝试 DrissionPage/Playwright 自动点击
    """
    # 1) 优先使用仓库 Secret（最稳）
    ...
    if secret_token:
        ...
        if re.match(r"^[a-f0-9]{16}$", secret_token, re.I):
            logger.info(f"从仓库 Secret 读取到 Token: {secret_token}")
            if _verify_token_once(secret_token):
                return secret_token
            else:
                logger.warning("仓库 Secret 中的 Token 已失效，将尝试其他方式")
        else:
            logger.warning("仓库 Secret 格式不正确，将尝试其他方式")

    # 2) CI 环境下不再尝试自动点击（Cloudflare 挑战页几乎必定拦截）
    if is_running_in_ci():
        logger.error("❌ CI 环境且未配置有效 REPO_SECRET_TAOTOKEN，无法获取 Token（已跳过自动化以避免被拦截）")
        return None

    # 3) 非 CI 环境尝试 DrissionPage -> Playwright
