...
def _ensure_playwright() -> bool:
    ...
    logger.info(" playwright 未安装，开始自动安装...")

    # 第 1 步：pip install playwright
    try:
        logger.info(" [1/3] pip install playwright ...")
        subprocess.check_call(
            [sys.executable, "-m", "pip", "install", "playwright", "-q"],
            stdout=subprocess.DEVNULL, stderr=subprocess.PIPE,
        )
        logger.info(" [1/3] playwright pip 包安装成功")
    except Exception as e:
        logger.error(f" [1/3] pip install playwright 失败: {e}")
        return False

    # 第 2 步：playwright install chromium（浏览器二进制）
    try:
        logger.info(" [2/3] playwright install chromium ...")
        subprocess.check_call(
            [sys.executable, "-m", "playwright", "install", "chromium"],
            stdout=subprocess.DEVNULL, stderr=subprocess.PIPE,
        )
        logger.info(" [2/3] chromium 浏览器下载完成")
    except Exception as e:
        logger.error(f" [2/3] 安装 chromium 失败: {e}")
        return False

    # 第 3 步：安装系统依赖（需要 sudo，GitHub Actions ubuntu runner 支持）
    try:
        logger.info(" [3/3] playwright install-deps chromium ...")
        subprocess.check_call(
            [sys.executable, "-m", "playwright", "install-deps", "chromium"],
            stdout=subprocess.DEVNULL, stderr=subprocess.PIPE,
        )
        logger.info(" [3/3] 系统依赖安装完成")
    except Exception as e:
        logger.warning(f" [3/3] install-deps 失败（可能无 root 权限，继续尝试）: {e}")

    # 验证安装
    try:
        from playwright.sync_api import sync_playwright  # noqa
        logger.info(" playwright 安装验证通过")
        return True
    except Exception as e:
        logger.error(f" playwright 安装验证失败: {e}")
        return False
...
def get_taoiptv_token() -> Optional[str]:
    logger.info("正在获取 TaoIPTV Token ...")

    if not _ensure_playwright():
        logger.error(" playwright 不可用且自动安装失败，无法获取 Token")
        return None

    try:
        from playwright.sync_api import sync_playwright
    except Exception as e:
        logger.error(f" 导入 playwright 失败: {e}")
        return None
    ...
    try:
        with sync_playwright() as p:
            ...
            if not clicked:
                # 尝试 JS 方式触发（如果按钮被隐藏或需要特殊交互）
                logger.info(" 未找到可见按钮，尝试 JS 触发 ...")
                js_result = page.evaluate("""async () => {
                    // 尝试找到包含"获取"或"Token"的可点击元素并点击
                    const els = document.querySelectorAll('a, button, span, div, p, h1, h2, h3, h4, li, td');
                    for (const el of els) {
                        if (el.offsetParent !== null && (el.textContent.includes('获取') || el.textContent.toLowerCase().includes('token'))) {
                            el.click();
                            return el.textContent.trim().substring(0, 100);
                        }
                    }
                    // 兜底：执行页面所有 script 里的 token 生成逻辑
                    // 返回页面标题和 body 前 200 字供诊断
                    return document.title + '|||' + document.body.innerText.substring(0, 200);
                }""")
                if js_result:
                    logger.info(f" JS触发结果: {str(js_result)[:200]}")

            page.wait_for_timeout(2000)

            # 读取截获的 Token
            token_raw = page.evaluate("() => window.__capturedToken || ''")
            token = None
            if token_raw:
                token_raw = str(token_raw).strip()
                m = re.search(r"\b([a-f0-9]{16})\b", token_raw, re.I)
                if m:
                    token = m.group(1)
                ...
                if body_text:
                    for line in body_text.splitlines():
                        ...
                        if "token" in low:
                            ...
                            if m:
                                token = m.group(1)
                                logger.info(f" 从页面文本提取到 Token（行: {line.strip()[:80]}）")
                                break

            # 第二次截图（点击后）
            debug_path2 = os.path.join(SCRIPT_DIR, "debug_token_after.png")
            try:
                page.screenshot(path=debug_path2)
                logger.info(f" 点击后截图已保存: {debug_path2}")
            except Exception:
                pass

            # 如果还是没拿到，打印页面关键信息帮助诊断
            if not token:
                title = page.evaluate("() => document.title || ''")
                snippet = page.evaluate("() => document.body ? document.body.innerText.substring(0, 500) : ''")
                logger.warning(f" 未获取到 Token")
                logger.warning(f" 页面标题: {title}")
                logger.warning(f" 页面文本: {snippet[:300]}")
        ...
        logger.error(f" Playwright 异常: {e}", exc_info=True)
        return None
...
def update_my_urls_all(token: str) -> bool:
    ...
    try:
        ...
        with open(file_path, "w", encoding="utf-8") as f:
            ...
            f.flush()
            os.fsync(f.fileno())
        logger.info(f" my_urls.txt 更新成功！替换 {count} 个 Token")
        return True
    except Exception as e:
...
def fetch_first_remote_source_to_111txt() -> bool:
    ...
    first_url = lines[0]
    logger.info(f" 抓取第一个远程源: {first_url}")
...
# 媒体类型判定
# ==============================================
STREAM_CTS = ["video/mp2t", "video/mp4", "video/x-flv", "application/vnd.apple.mpegurl",
        ...
        for raw_url in urls:
            ...
            try:
                ...
                if got > 1:
                    ...
                    logger.warning(f" ⚠ {raw_url[:90]} → 仅 {got} 个源 | {c[:200].replace(chr(10), ' ')}")
                else:
                    logger.warning(f" ✗ {raw_url[:90]} → 0 个源")
            except Exception as e:
...
def main():
    try:
        ...
        if not token:
            logger.warning("未获取到 Token，跳过 my_urls.txt / 111.txt 更新")
        else:
