...
def get_taoiptv_token() -> Optional[str]:
    """纯Python原生库实现，绕过Cloudflare获取Token"""
    try:
...
def update_my_urls_all(token: str) -> bool:
    """批量更新my_urls.txt中所有链接的Token，兼容纯URL行"""
...
# ==============================================
# 【核心修复】增强版111.txt处理逻辑
# 兼容「频道名,URL」格式和纯URL格式，双重解析确保能拿到有效URL
# ==============================================
def fetch_first_source_to_111txt():
    """
    增强版解析逻辑：
    1. 跳过注释行、空行，逐行解析
    2. 优先用原项目clean_source_line解析（兼容逗号分隔格式）
    3. 解析失败则用正则直接提取行内URL（兼容纯URL行）
    4. 详细日志输出，方便排查问题
    """
    ...
    # 第二步：读取my_urls.txt全量内容，打印预览方便排查
    try:
        with open(my_urls_path, "r", encoding="utf-8") as f:
            raw_content = f.read()
            lines = raw_content.splitlines()
        # 打印文件内容预览，方便排查格式问题
        logger.info(f"📄 my_urls.txt内容预览（前10行）:\n{raw_content[:500]}")
    except Exception as e:
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        if not line_strip or line_strip.startswith("#"):
            logger.info(f"⏭️  跳过第{line_num}行: 空行/注释行")
            continue
        
        logger.info(f"🔍 正在解析第{line_num}行: {line_strip[:100]}")
        
        # 解析逻辑1：优先用原项目的clean_source_line（兼容「频道名,URL」格式）
        res, status = clean_source_line(line_strip)
        if status == CLEAN_OK and res:
            name, url = res
            first_valid_url = url
            valid_line_num = line_num
            logger.info(f"✅ 第{line_num}行解析成功（标准格式） | 频道名: {name} | URL: {first_valid_url}")
            break
        
        # 解析逻辑2：兜底正则提取，直接拿行内的http/https链接（兼容纯URL行）
        url_match = RE_ALL_URLS.search(line_strip)
        if url_match:
            candidate_url = url_match.group(0).rstrip(".,;:!?)")
            # 过滤黑名单、点播/图片链接，和原项目逻辑保持一致
            if not url_matches_domain_blacklist(candidate_url) and not is_vod_or_image_url(candidate_url):
                first_valid_url = candidate_url
                valid_line_num = line_num
                logger.info(f"✅ 第{line_num}行解析成功（纯URL格式） | URL: {first_valid_url}")
                break
            else:
                logger.warning(f"⚠️  第{line_num}行提取到URL，但在黑名单/是点播图片链接，跳过")
...
"""
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            f.flush()
            os.fsync(f.fileno())
        logger.error("❌ 111.txt更新失败: 未解析到有效URL，已写入详细排查信息到111.txt")
        return False

    # 第四步：抓取URL内容
    try:
        logger.info(f"📥 开始抓取第{valid_line_num}行的远程源内容: {first_valid_url}")
        ctx = ssl._create_unverified_context()
