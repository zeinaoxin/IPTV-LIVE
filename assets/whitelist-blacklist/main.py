...
ASSETS_DIR = os.path.dirname(SCRIPT_DIR)               # 即 assets 目录
...
FILE_PATHS = {
    ...
    "first_parse": os.path.join(ASSETS_DIR, "111.txt"),   # 第一个远程源解析内容，路径: assets/111.txt
}
...
def git_commit_push():
    try:
        ...
        # 添加 my_urls.txt 和 assets/111.txt（如果存在）
...
def main():
    try:
        ...
        if token:
            ...
            logger.warning("⚠️ 无法获取新Token，将使用现有地址继续")

        # 强制解析第一个远程源并保存到 assets/111.txt
        logger.info("开始处理第一个远程源，目标文件：assets/111.txt")
        first_url = get_first_my_url()
        out_path = FILE_PATHS["first_parse"]          # assets/111.txt
        bj = datetime.now(timezone.utc) + timedelta(hours=8)
        ...
        with open(out_path, "w", encoding="utf-8") as f:
            ...
            if first_url:
                ...
                logger.info(f"正在获取远程内容: {first_url[:90]}...")
                content = fetch_url_content(first_url)
                if content:
                    if "Authentication Failed" in content or "认证token参数不正确" in content:
                        f.write(f"# 警告: 远程服务器认证失败，Token可能已过期或编码有误\n")
                        f.write("# 原始返回（前10行）:\n")
                    ...
                    f.write(f"# 错误: 无法获取远程源内容\n")
                    logger.warning("⚠️ 远程源内容为空，已写入错误提示")
            else:
