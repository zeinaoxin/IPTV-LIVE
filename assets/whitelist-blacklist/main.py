...
# ===================== 【优化】一次性获取Token =====================
def get_token_once() -> Optional[str]:
    """只获取一次Token，简单直接"""
    try:
        logger.info("正在获取TaoIPTV Token...")
        ...
        with urllib.request.urlopen(req, context=ctx, timeout=15) as resp:
            if resp.getcode() != 200:
                logger.error(f"访问失败，状态码: {resp.getcode()}")
                return None
...
# ===================== 【优化】一次性更新所有链接的Token =====================
def update_all_tokens(token: str) -> bool:
    """用同一个Token批量更新所有链接"""
    ...
    if not os.path.exists(my_urls_path):
        logger.error(f"❌ 文件不存在: {my_urls_path}")
        return False
    ...
    try:
        ...
        # 统计需要替换的数量
        old_tokens = re.findall(r'token=[a-f0-9]{16}', content, re.I)
        if not old_tokens:
        ...
        logger.info(f"✅ 成功更新 {len(old_tokens)} 个链接的Token")
        return True
    except Exception as e:
...
def main():
    try:
        ...
        # 【优化】第二步：用同一个Token更新所有链接
        if token:
            update_all_tokens(token)
        
        # 第三步：继续原来的WhiteList BlackList流程
