...
def _fetch_taoiptv_token():
    """
    爬取 https://www.taoiptv.com 首页源码，关闭 SSL 证书验证，
    用正则 [a-f0-9]{16} 提取 16 位十六进制字符作为 Token。
    """
    try:
        # 关闭 SSL 校验
        ...
        logger.warning("taoiptv 首页未匹配到 [a-f0-9]{16} 的 Token")
        return None
...
def _refresh_taoiptv_token():
    """
    获取新 Token，并统一替换 assets/my_urls.txt 中所有
    https://taoiptv.com/.../xxx.txt?token=<旧token> 中的 <旧token>，
    然后保存回文件。
    """
    new_token = _fetch_taoiptv_token()
    if not new_token:
        logger.warning("未获取到 taoiptv Token，跳过 my_urls.txt 的 token 刷新")
        return
    ...
    if not os.path.isfile(my_urls_path):
        logger.warning(f"my_urls.txt 不存在: {my_urls_path}")
        return

    with open(my_urls_path, "r", encoding="utf-8") as f:
        content = f.read()

    # 匹配：... taoiptv.com/....txt?token=旧token（旧token 可以是十六进制字母数字）
    new_content = re.sub(
        r"(https?://[^ \t\n\r]*taoiptv\.com/[^?]*\?token=)[a-fA-F0-9]+",
        r"\g<1>" + new_token,
        content,
        flags=re.IGNORECASE,
    )
    ...
    if changed:
        with open(my_urls_path, "w", encoding="utf-8") as f:
            f.write(new_content)
        logger.info(f"已更新 {my_urls_path} 中的 taoiptv Token 为 {new_token}")
    else:
        logger.info(f"{my_urls_path} 中未检测到需要更新的 taoiptv.com token 链接")
...
class StreamChecker:
    ...
    def run(self):
        ...
        # ====== 新增：先刷新 taoiptv Token 并更新 my_urls.txt ======
