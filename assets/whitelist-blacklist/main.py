# main.py（仓库根目录的 main.py）
# 注意：本文件位于仓库根目录，与原仓库里的 main.py 位置一致。
...
def get_file_paths():
    ...
    return {
        ...
        "blacklist_auto": os.path.join(current_dir, 'blacklist_auto.txt'),  # 若根目录无此文件可忽略
...
# ===================== 提前刷新 taoiptv Token（仅修改 assets/my_urls.txt） =====================
try:
    from utils_taoiptv_token import refresh_taoiptv_token
    logger.info("尝试刷新 taoiptv Token 并更新 assets/my_urls.txt ...")
    new_token = refresh_taoiptv_token()
    if new_token:
        logger.info(f"taoiptv Token 已刷新: {new_token}")
    else:
        logger.warning("taoiptv Token 刷新失败或未命中链接，继续执行主流程。")
except Exception as e:
    logger.warning(f"taoiptv Token 刷新模块异常（不影响主流程）: {e}")
