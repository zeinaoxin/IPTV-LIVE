...
import opencc
import time
import concurrent.futures  # 【新增】用于并发测速
...
# ===================== 黑名单特征集（轻量级快速拦截） =====================
BLOCK_FEATURES = (
    "[", "【", "catvod.com", "公众号", "kkk.jjjj.jiduo.me", "freetv.top", "freetv.fun",
    "hwrr.jx.chinamobile.com", "kkk.888.3116598.xyz", "p3p://", "rtsp://", "/tsfile/live/",
    "iptv.cdn.ha.chinamobile.com", "www.douzhicloud.site", "e1.woc.iiiya.cn", "php.jdshipin.com",
    "dbiptv.sn.chinamobile.com", "www.zzlag.top", "chinamobile.com/PLTV/88888888/8",
    "hlsztemgsplive.miguvideo.com", "api.mytv666.top", "live.sxrtv.com", "hls-gateway.vpstv.net"
)
...
def load_channel_dictionaries(main_dir: str, local_dir: str) -> tuple:
    ...
    local_channels = {
        "儿童频道": "儿童频道.txt", "国际台": "国际台.txt", "纪录片": "纪录片.txt", "戏曲频道": "戏曲频道.txt",
        "上海频道": "上海频道.txt", "湖南频道": "湖南频道.txt", "湖北频道": "湖北频道.txt", "广东频道": "广东频道.txt",
        "浙江频道": "浙江频道.txt", "山东频道": "山东频道.txt", "江苏频道": "江苏频道.txt", "安徽频道": "安徽频道.txt",
        "海南频道": "海南频道.txt", "内蒙频道": "内蒙频道.txt", "辽宁频道": "辽宁频道.txt", "陕西频道": "陕西频道.txt",
        "山西频道": "山西频道.txt", "云南频道": "云南频道.txt", "北京频道": "北京频道.txt", "重庆频道": "重庆频道.txt",
        "福建频道": "福建频道.txt", "甘肃频道": "甘肃频道.txt", "广西频道": "广西频道.txt", "贵州频道": "贵州频道.txt",
        "河北频道": "河北频道.txt", "河南频道": "河南频道.txt", "黑龙江频道": "黑龙江频道.txt", "吉林频道": "吉林频道.txt",
        "江西频道": "江西频道.txt", "宁夏频道": "宁夏频道.txt", "青海频道": "青海频道.txt", "四川频道": "四川频道.txt",
        "天津频道": "天津频道.txt", "新疆频道": "新疆频道.txt"
    }
    local_dict = {}
...
def convert_m3u_to_txt(m3u_content: str) -> list:
    ...
    for line in lines:
        if line.startswith("#EXTINF"):
            m = re.search(r',(.+)$', line)
            if m: channel_name = m.group(1).strip()
...
def process_single_line(line: str, classifier: ChannelClassifier, corrections: dict):
    ...
    if any(block in line for block in BLOCK_FEATURES): return
    
    idx = line.rfind(',')
    channel_name_raw = line[:idx].strip()
    channel_url_raw = line[idx+1:].strip()
    if not channel_url_raw or '://' not in channel_url_raw: return
...
def generate_live_text(classifier: ChannelClassifier, main_dict: dict) -> tuple:
    ...
    while full_lines and not full_lines[-1]: full_lines.pop()
    while lite_lines and not lite_lines[-1]: lite_lines.pop()
...
# ===================== 【新增】电影频道测速模块 =====================
def test_single_url(item: str, timeout: int = 3) -> bool:
    """测试单条 URL 是否可连通"""
    try:
        if "," not in item: return False
        url = item.split(",", 1)[1]
        req = urllib.request.Request(url, headers={"User-Agent": USER_AGENT})
        # 超时时间设为 3 秒，能连上就说明源可用
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            return resp.status == 200
    except Exception:
        return False

def filter_movie_channels(movie_list: list) -> list:
    """多线程并发测速，筛除无效电影源"""
    valid_items = []
    # 开 50 个线程并发，速度极快
    with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
        future_to_item = {executor.submit(test_single_url, item): item for item in movie_list}
        for future in concurrent.futures.as_completed(future_to_item):
            ...
            try:
                if future.result():
                    ...
                    # 打印被淘汰的源方便排查
                    print(f"[SPEED-TEST] 淘汰无效源: {item.split(',')[1]}")
            except Exception:
                pass
    return valid_items
...
if __name__ == "__main__":
    ...
    # 3.5) 【新增】仅对“电影频道”进行测速过滤
    print(f"[PROCESS] ===== 开始对【电影频道】进行测速过滤 =====")
    movie_data = classifier.channel_data.get("电影频道", [])
    if movie_data:
        original_count = len(movie_data)
        valid_movies = filter_movie_channels(movie_data)
        classifier.channel_data["电影频道"] = valid_movies
        print(f"[PROCESS] ===== 电影频道测速完成：保留 {len(valid_movies)}/{original_count} 条有效源 =====")
    else:
        print(f"[PROCESS] ===== 电影频道无源，跳过测速 =====")
